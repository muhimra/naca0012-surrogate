...
| Metric | Value |
|---|---|
| Cl R² (held-out CFD test set) | 0.988 |
| Cd R² (held-out CFD test set) | 0.993 |
| Cl error vs NASA TM-4071 (α<14°) | 1.5% mean |
| Cl error vs NASA TM-4071 (all α) | 5.0% mean |
| Cd error vs NASA TM-4071 | 31.7% (k-ω SST limitation) |
| Surrogate prediction time | 0.11 ms per query |
| CFD run time (assumed) | ~10 min |
| **Speedup factor** | **~5,000,000 x (estimate)** |
...
- **Surrogate accuracy:** held-out Cl R² = 0.988, Cd R² = 0.993 (10-point untouched test set)
- **Speed:** 0.11 ms per prediction vs ~10 minutes per CFD run (~5M x speedup, estimate)
- **Cl validation:** 1.5% mean error vs NASA experimental data for attached flow (α < 14°); 5.0% across all angles
- **Cd deviation:** 31.7% vs NASA — attributable to k-omega SST turbulence model
